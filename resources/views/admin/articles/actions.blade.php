<div class="d-flex">
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center edit-btn" type="button" title="Edit"
        ...
        data-update-route="{{ route('admin.article.store') }}"
        data-row-data="{{ json_encode([$article->id, $article->category_id, $article->title, $article->content]) }}">
    ...
    <button class="px-2 py-2 btn btn-link nav-link d-flex align-items-center" type="button" title="Delete"
        ...
        data-delete-route="{{ route('admin.article.destroy', $article->id) }}">
